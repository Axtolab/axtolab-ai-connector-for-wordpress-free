...
class Axtolab_AI_Connector_OAuth {
	...
	public static function handle_register( WP_REST_Request $request ): WP_REST_Response {
		...
		$rate_key = 'axtolab_ai_connector_dcr_' . md5( $ip );
		...
		// Generate client_id.
		$client_id = 'axtolab_ai_connector_cid_' . wp_generate_uuid4();
		...
		set_transient( 'axtolab_ai_connector_oauth_client_' . $client_id, $client_data, self::CLIENT_REGISTRATION_EXPIRY );
	...
	public static function handle_authorize_get_direct(): void {
		...
		$nonce       = wp_create_nonce( 'axtolab_ai_connector_oauth_authorize' );
	...
	public static function handle_authorize_post_direct(): void {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( $post_params['_wpnonce'] ?? '' ), 'axtolab_ai_connector_oauth_authorize' ) ) {
	...
	public static function handle_token( WP_REST_Request $request ): WP_REST_Response {
		...
		$code_data = get_transient( 'axtolab_ai_connector_oauth_code_' . $code );
		delete_transient( 'axtolab_ai_connector_oauth_code_' . $code );
	...
	private static function handle_refresh_token( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( strpos( $refresh_token, 'axtolab_ai_connector_ort_' ) !== 0 ) {
	...
	public static function verify_access_token( string $token ): bool {
		...
		if ( strpos( $token, 'axtolab_ai_connector_oat_' ) !== 0 ) {
			return false;
		}
