...
* **Secure by Design** — Dedicated service account with minimal permissions. Confirmation tokens required for publish, trash, and restore operations. Allowlist-driven content type and taxonomy controls. Rate limiting on authentication endpoints.
* **Multiple Auth Methods** — Application Passwords, Device Authorization (RFC 8628), OAuth 2.1 with PKCE, and Bearer Token for MCP-over-HTTP transport.
...
* All authentication methods (App Passwords, Device Auth, OAuth 2.1, Bearer Token)
...
3. Download the `.mcpb` extension from the setup page and install it in Claude Desktop, or connect a compatible client through MCP-over-HTTP
...
3. Go to Settings > AI Connector to configure

The plugin automatically creates a dedicated service account (`axtolab-connector-service`) with the `axtolab_ai_connector_editor` role. No manual user setup is needed.

= About the bundled `.mcpb` file =

The `assets/axtolab-ai-connector.mcpb` file inside this plugin's `assets/` directory is the Claude Desktop installer bundle (a Claude Desktop "MCP bundle" extension). It is a passive download made available to site administrators who choose to connect Claude Desktop to their site — the plugin does not execute it. The file is served as a static asset only when an administrator clicks the "Download Extension (.mcpb)" button in the AI Connector setup page; nothing is auto-installed, auto-run, or sent off-site by the plugin itself.
...
The plugin supports four authentication methods: WordPress Application Passwords, Device Authorization (RFC 8628), OAuth 2.1 with PKCE, and Bearer Tokens. All methods create or use a dedicated service account with minimal permissions.
...
* **Device Authorization** (RFC 8628) — same scope as Application Password; the device flow simply replaces the manual Application Password creation step with an in-browser approval.
...
3. Device authorization tab for compatible clients that display a one-time authorization code.
...
* **Four authentication methods.** Application Passwords (HTTP Basic), Device Authorization (RFC 8628), OAuth 2.1 with PKCE S256 + dynamic client registration (RFC 7591), and Bearer Token for MCP-over-HTTP transport.
...
* **Submission readiness pass** — feature settings stand alone (no inline upsells), all third-party services disclosed in the External Services section, the bundled `.mcpb` Claude Desktop installer is described under Installation as a passive download.
