 ...
 * including the "Connect AI Client" device-authorization section, setup
...
class Axtolab_AI_Connector_Admin {
	...
	/**
	 * AJAX action for authorizing a device via user_code.
	 *
	 * @var string
	 */
	const AJAX_AUTHORIZE = 'axtolab_ai_connector_authorize_device';
	...
	public function init(): void {
		...
		add_action( 'wp_ajax_' . self::AJAX_AUTHORIZE, array( $this, 'ajax_authorize_device' ) );
	...
	public function enqueue_assets( string $hook_suffix ): void {
		...
		wp_localize_script(
			...
			array(
				...
				'strings'        => array(
					'authorizing'  => __( 'Authorizing…', 'axtolab-ai-connector' ),
				...
				'mcpbAvailable'  => file_exists( AXTOLAB_AI_CONNECTOR_DIR . 'assets/axtolab-ai-connector.mcpb' ),
				'actions'        => array(
...
JS;
	...
	/**
	 * Render the "Connect AI Client" card with the user_code input and management tools.
	 *
	 * @param  array $status Result of {@see get_setup_status()}.
	 * @return void
	 */
	private function render_connect_claude_section( array $status ): void {
		...
		<div class="mcp-gateway-card mcp-gateway-connect">
			...
			<div class="mcp-tabs">
				...
				<button type="button" class="mcp-tab" data-tab="terminal">
					<?php esc_html_e( 'Device Authorization', 'axtolab-ai-connector' ); ?>
				</button>
			...
			<div class="mcp-tab-content mcp-tab-content-active" data-tab="quick-connect">
				...
				<p class="mcp-help-text"><?php esc_html_e( 'Set up Claude Desktop, Claude Code, or compatible local AI clients for the best experience — full filesystem access, local image uploads, and one-click device authorization.', 'axtolab-ai-connector' ); ?></p>

				<?php
				$mcpb_file = AXTOLAB_AI_CONNECTOR_DIR . 'assets/axtolab-ai-connector.mcpb';
				$mcpb_available = file_exists( $mcpb_file );
				?>

				<?php if ( $mcpb_available ) : ?>
				...
				<p class="mcp-help-text">
					<?php esc_html_e( 'Download and drag into Claude Desktop\'s Extensions panel.', 'axtolab-ai-connector' ); ?>
				</p>
				<p>
					<a href="<?php echo esc_url( wp_nonce_url( rest_url( 'axtolab-ai-connector/v1/extension/download' ), 'wp_rest' ) ); ?>" class="button button-secondary" id="mcp-download-mcpb-btn" download>
				...
				<?php else : ?>
				<p class="mcp-field-label"><?php esc_html_e( 'Step 1: Generate a connection token', 'axtolab-ai-connector' ); ?></p>
				<?php endif; ?>
				<p class="mcp-help-text">
				...
				<div id="mcp-token-result" style="display:none;">
					<p class="mcp-field-label"><?php echo esc_html( $mcpb_available ? __( 'Step 3: Paste the token into the extension settings', 'axtolab-ai-connector' ) : __( 'Step 2: Copy this for Claude Desktop', 'axtolab-ai-connector' ) ); ?></p>
					<?php if ( $mcpb_available ) : ?>
					<p class="mcp-help-text">
			...
			<?php // ── Tab 2: Terminal (device auth, requires HTTP) ────────────────── ?>
			<div class="mcp-tab-content" data-tab="terminal">

				<p class="mcp-help-text"><?php esc_html_e( 'For compatible clients that support device authorization. Your client will show a short code; enter it here to approve access to this WordPress site.', 'axtolab-ai-connector' ); ?></p>

				<p class="mcp-field-label"><?php esc_html_e( 'Enter the authorization code from your AI client', 'axtolab-ai-connector' ); ?></p>

				<div class="mcp-gateway-authorize-form">
					<div class="mcp-input-row">
						<input
							type="text"
							id="mcp-user-code"
							class="regular-text mcp-user-code-input"
							placeholder="ABCD-EFGH"
							maxlength="9"
							autocomplete="off"
							spellcheck="false"
						/>
						<button type="button" id="mcp-authorize-btn" class="button button-primary">
							<?php esc_html_e( 'Authorize', 'axtolab-ai-connector' ); ?>
						</button>
					</div>
					<p id="mcp-authorize-message" class="mcp-feedback" aria-live="polite"></p>
				</div>

			</div><!-- tab: terminal -->

			<?php // ── Tab 3: Remote Access (bearer token + OAuth) ── ?>
			<?php
	...
	private function render_info_section(): void {
		...
		<div class="mcp-gateway-card mcp-gateway-info">
			...
			<h3><?php esc_html_e( 'Device Authorization', 'axtolab-ai-connector' ); ?></h3>
			<ol>
				<li><?php esc_html_e( 'Start the device authorization flow in a compatible AI client.', 'axtolab-ai-connector' ); ?></li>
				<li><?php esc_html_e( 'Enter the displayed code in the Device Authorization tab and click Authorize.', 'axtolab-ai-connector' ); ?></li>
				<li><?php esc_html_e( 'The client completes the connection automatically after approval.', 'axtolab-ai-connector' ); ?></li>
			</ol>
			<p>
				<?php
				printf(
	...
	/**
	 * AJAX: Authorize a device using a user_code.
	 *
	 * Proxies to the REST endpoint `POST /device/authorize` internally so
	 * that the same validation logic is reused without duplication.
	 *
	 * Nonce: `{MENU_SLUG}-ajax` (standard wp_ajax nonce pattern).
	 *
	 * @return void Sends JSON and exits.
	 */
	public function ajax_authorize_device(): void {
		check_ajax_referer( self::MENU_SLUG . '-ajax', 'nonce' );
		...
		if ( empty( $user_code ) ) {
			wp_send_json_error(
				array( 'message' => __( 'Please enter a code.', 'axtolab-ai-connector' ) ),
				400
			);
		}

		// Call the REST handler directly.
		$device_auth = new Axtolab_AI_Connector_Device_Auth();
		$fake_request = new WP_REST_Request( 'POST', '/axtolab-ai-connector/v1/device/authorize' );
		$fake_request->set_body_params( array( 'user_code' => $user_code ) );

		$response = $device_auth->handle_device_authorize( $fake_request );
...
(function ($) {
    ...
    // ── User-code input: live uppercase + auto-hyphen ─────────────────────────
    $(document).on('input', '#mcp-user-code', function () {
        var val    = $(this).val().toUpperCase().replace(/[^A-Z0-9]/g, '');
        var cursor = this.selectionStart;
        var out    = val.slice(0, 8);
        if (out.length > 4) {
    ...
    // ── Authorize button ─────────────────────────────────────────────────────
    $(document).on('click', '#mcp-authorize-btn', function () {
        var $btn     = $(this);
        var $msg     = $('#mcp-authorize-message');
        var userCode = $('#mcp-user-code').val().trim();

        if (!userCode) {
            $msg.text('Please enter the code from your terminal.').removeClass('is-success').addClass('is-error');
            return;
        }

        $btn.prop('disabled', true).text(cfg.strings.authorizing);
        $msg.text('').removeClass('is-success is-error');

        doAjax(
            'axtolab_ai_connector_authorize_device',
            { user_code: userCode },
            function (data) {
                $btn.prop('disabled', false).text('Authorize');
                $msg.text(data.message || 'Authorized!').removeClass('is-error').addClass('is-success');
                $('#mcp-user-code').val('');
                // Reload after short delay so the checklist reflects the new connection.
