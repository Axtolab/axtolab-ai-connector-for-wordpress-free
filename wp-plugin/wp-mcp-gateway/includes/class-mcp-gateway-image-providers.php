...
final class Axtolab_AI_Connector_Image_Providers {
	...
	public static function import_stock_photo( string $provider, string $provider_id, array $options = array() ) {
		...
		update_post_meta( $attach_id, '_mcp_image_provider', $provider );
		update_post_meta( $attach_id, '_mcp_image_provider_id', $provider_id );
		update_post_meta( $attach_id, '_mcp_image_attribution', $attribution );
	...
	private static function save_to_media_library( string $bytes, string $format, string $prompt, string $provider ) {
		...
		update_post_meta( $attach_id, '_mcp_image_provider', $provider );
		update_post_meta( $attach_id, '_mcp_image_prompt', $prompt );
		update_post_meta( $attach_id, '_mcp_image_status', 'pending' );
		update_post_meta( $attach_id, '_mcp_image_generated_at', current_time( 'mysql', true ) );
	...
	public static function confirm_image( int $media_id ): bool {
		$status = get_post_meta( $media_id, '_mcp_image_status', true );
		if ( 'pending' !== $status ) {
			return false;
		}
		update_post_meta( $media_id, '_mcp_image_status', 'confirmed' );
		return true;
	}
	...
	public static function cleanup_pending_images(): void {
		$query = new WP_Query( array(
			...
			'meta_query'     => array(
				'relation' => 'AND',
				array(
					'key'   => '_mcp_image_status',
					'value' => 'pending',
				),
				array(
					'key'     => '_mcp_image_generated_at',
