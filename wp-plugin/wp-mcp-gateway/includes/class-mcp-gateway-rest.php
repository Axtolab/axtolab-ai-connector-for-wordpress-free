...
final class Axtolab_AI_Connector_REST {
	...
	/**
	 * Serve the .mcpb extension bundle for Claude Desktop.
	 *
	 * @param WP_REST_Request $request
	 * @return WP_REST_Response|WP_Error
	 */
	public static function handle_extension_download( WP_REST_Request $request ) {
		$file = AXTOLAB_AI_CONNECTOR_DIR . 'assets/axtolab-ai-connector.mcpb';

		if ( ! file_exists( $file ) ) {
			return new WP_Error(
				'mcpb_not_found',
				'The extension bundle is not available. Please rebuild the plugin package.',
				array( 'status' => 404 )
			);
		}
		...
		header( 'Content-Disposition: attachment; filename="axtolab-ai-connector.mcpb"' );
		...
		header( 'Pragma: no-cache' );
			...
			if ( $wp_filesystem ) {
				echo $wp_filesystem->get_contents( $file ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Binary .mcpb download.
			}
			exit;
		}
