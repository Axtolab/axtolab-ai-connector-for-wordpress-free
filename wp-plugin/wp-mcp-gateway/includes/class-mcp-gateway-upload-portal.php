...
class Axtolab_AI_Connector_Upload_Portal {
	...
	const TRANSIENT_PREFIX  = '_mcp_upload_session_';
	const SID_PREFIX        = '_mcp_upload_sid_';
	const NONCE_ACTION      = 'mcp_upload_portal';
	...
	public static function create_session( $created_by, $client_ip = null ) {
		...
		$rate_key = 'mcp_upload_rate_' . $created_by;
	...
	public static function handle_upload( $token, $file, $client_ip = null ) {
		...
		if ( ! $service_user_id || ! get_user_by( 'id', $service_user_id ) ) {
			...
			self::debug_log( 'MCP Upload Portal: Service account not found (ID: ' . $service_user_id . '). Run plugin setup.' );
			return new WP_Error( 'service_account_missing', 'Upload portal not configured. Please deactivate and reactivate the plugin.', array( 'status' => 500 ) );
		}
		...
		update_post_meta( $attachment_id, '_mcp_upload_session', $session['session_id'] );
