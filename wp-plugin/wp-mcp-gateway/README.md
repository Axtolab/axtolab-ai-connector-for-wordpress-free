...
WordPress built-in Application Passwords via HTTP Basic Auth. Created automatically during device authorization setup.

### 2. Device Authorization (Recommended Setup)
```bash
npx axtolab-ai-connector --setup
```
Enter the displayed code in Settings → AI Connector → Connect AI Client. An Application Password is created for the `axtolab-connector-service` account automatically.
...
- **Connect AI Client** — device authorization code entry
- **Service Account** — status of the `axtolab-connector-service` user
...
- **Rate limiting** — per-IP limits on device auth and OAuth registration
...
- All plugin transients (device codes, upload sessions, rate limits, OAuth tokens)
